...
export const ChatHeader: FC<IChatHeaderProps> = ({ to }) => {
	return (
		<>
			<strong className='me-2'>Observações do Trabalho:</strong>
			{to}
		</>
	);
};
		...
		<div className='chat-messages' {...props}>
			{messages.map((i) => (
				<div
					...
					{/* Exibir o nome do usuário antes da mensagem */}
					<small
						className={classNames('d-block', {
							'text-primary': !isReply, // Cor azul para mensagens normais
							'text-white': isReply, // Cor branca para mensagens de resposta
						})}>
					...
					{/* Exibir a data associada à mensagem */}
					<small className='text-muted d-block text-end'>{i.data}</small>
				</div>
			))}
		</div>
	);
...
export const ChatGroup: FC<IChatGroupProps> = ({
	...
	return (
		...
		<div className={classNames('chat-group', { 'chat-group-reply': isReply })} {...props}>
			...
			<ChatMessages
				messages={messages.map((msg) => ({ ...msg, data: msg.data || '' }))}
				isReply={isReply}
			/>
